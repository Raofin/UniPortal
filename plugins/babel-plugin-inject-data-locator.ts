  ...
  return {
    ...
    pre(file: BabelFile) {
      ...
      if (options.inputSourceMap) {
        try {
          ...
          console.warn(`[inject-data-locator-original-source] Failed to initialize SourceMapConsumer for ${file.opts.filename}:`, error.message)
          consumerHolder.consumer = undefined
        }
      } else {
        consumerHolder.consumer = undefined
    ...
    visitor: {
      JSXElement(path: NodePath<BabelTypes.JSXElement>, state: BabelState) {
        ...
        const hasDataLocator = attributes.some((attr) => t.isJSXAttribute(attr) && attr.name.name === 'data-locator')
