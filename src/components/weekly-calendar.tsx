...
export const WeeklyCalendar: React.FC = () => {
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      case "present":
        return <span className="w-3 h-3 rounded-full bg-success-500"></span>;
      case "absent":
        return <span className="w-3 h-3 rounded-full bg-danger-500"></span>;
      case "holiday":
        return <span className="w-3 h-3 rounded-full bg-warning-500"></span>;
      default:
        return <span className="w-3 h-3 rounded-full bg-default-200"></span>;
    }
  };
            ...
            {week.map((day, index) => (
              ...
              >
                <Button
                  ...
                  className={`flex flex-col h-auto min-w-[80px] py-2 ${
      ...
      <CardBody>
        ...
        {selectedDay.classes.length > 0 ? (
          <div className="space-y-3">
            {selectedDay.classes.map((classItem, index) => (
              <motion.div
                ...
                className="flex items-center gap-3 p-3 rounded-lg border border-divider hover:bg-content1 transition-colors"
              >
          ...
          <div className="flex flex-col items-center justify-center py-8 text-center">
            <Icon icon="lucide:calendar-off" className="w-12 h-12 text-default-300 mb-2" />
